...
import { callDeepSeek, callGemini, callClaude, callClaudeCLI } from "./providers.mjs";
...
test("Claude CLI returns null when the binary is missing", async () => {
  assert.equal(await callClaudeCLI({ system: "s", user: "u", bin: "claude-does-not-exist-xyz" }), null);
});
